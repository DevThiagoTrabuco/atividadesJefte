...
public class EnderecoController {
    ...
    @PostMapping
    public ResponseEntity<?> criarEndereco(@RequestBody EnderecoEntity enderecoEntity) {
        try {
            enderecoService.criarEndereco(enderecoEntity);
    ...
    @PreAuthorize("hasRole('ADMIN') and hasRole('ATIVADO')")
    @PutMapping("/{id}")
    public ResponseEntity<?> atualizarEndereco(@PathVariable int id, @RequestBody EnderecoEntity enderecoEntity) {
        try {
            enderecoEntity.setId(id);
            enderecoService.atualizarEndereco(enderecoEntity);
    ...
    @PreAuthorize("hasRole('ADMIN') and hasRole('ATIVADO')")
    @GetMapping("/uf/{uf}")
    public List<EnderecoModel> buscarPorUnidadeFederativa(@PathVariable UnidadeFederativa uf) {
        return enderecoService.buscarPorUnidadeFederativa(uf);
    }

    @PreAuthorize("hasRole('ADMIN') and hasRole('ATIVADO')")
    @GetMapping("/cidade/{cidade}")
    public ResponseEntity<List<EnderecoModel>> buscarPorCidade(@PathVariable String cidade) {
